...
contract Debug {
    ...
    function decodePRH(bytes memory data) public view returns (ProviderRandomHash memory) {
        RLPReader.RLPItem[] memory prh = data.toRlpItem().toList();
        return ProviderRandomHash(toBytes32(prh[0].toBytes()), prh[1].toAddress(), prh[2].toUint());
    }
